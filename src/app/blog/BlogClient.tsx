...
const TRANSLATIONS = {
  ...
  bn: {
    ...
    noArticles: "আর্টিকেল পাওয়া যায়নি।",
  }
};
...
export default function BlogClient({ initialPosts }: BlogClientProps) {
  ...
  const filteredPosts = initialPosts.filter((post) => {
    const title = activeLang === "bn" ? post.titleBn || post.titleEn : post.titleEn;
    const excerpt = activeLang === "bn" ? post.excerptBn || post.excerptEn : post.excerptEn;
    const matchesSearch =
            ...
            <div className="space-y-10">
                  ...
                  <div className="relative h-60 md:h-80 lg:h-full lg:col-span-7 bg-neutral-100 overflow-hidden group">
                    {featuredPost.image ? (
                      <Image
                        ...
                        alt={activeLang === "bn" ? featuredPost.titleBn || featuredPost.titleEn : featuredPost.titleEn}
                        fill
                        priority
                    ...
                    <div className="space-y-4">
                      ...
                      <h2 className="font-kanit text-[24px] md:text-[28px] font-bold text-neutral-900 leading-snug hover:text-brand-red transition-colors">
                        <Link href={`/blog/${featuredPost.id}`}>
                          {activeLang === "bn" ? featuredPost.titleBn || featuredPost.titleEn : featuredPost.titleEn}
                        </Link>
                      </h2>
                      <p className="text-[15px] leading-relaxed text-neutral-600 font-medium">
                        {activeLang === "bn" ? featuredPost.excerptBn || featuredPost.excerptEn : featuredPost.excerptEn}
                      </p>
                    </div>

                    <div className="pt-4 border-t border-neutral-100 flex items-center justify-between gap-4">
                      <div className="text-[12px] text-neutral-500">
                        <span className="font-bold">{text.authorLabel} {activeLang === "bn" ? featuredPost.authorBn || featuredPost.authorEn : featuredPost.authorEn}</span>
              ...
              {gridPosts.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {gridPosts.map((post) => {
                    const displayTitle = activeLang === "bn" ? post.titleBn || post.titleEn : post.titleEn;
                    const displayExcerpt = activeLang === "bn" ? post.excerptBn || post.excerptEn : post.excerptEn;
                    const displayReadTime = activeLang === "bn" ? post.readTimeBn || post.readTimeEn : post.readTimeEn;
                        ...
                        <div>
                          ...
                          <div className="p-6 space-y-3">
                            <div className="flex items-center justify-between gap-4">
                              ...
                              <span className="text-[12px] text-neutral-400 font-semibold">{displayReadTime}</span>
                            </div>
