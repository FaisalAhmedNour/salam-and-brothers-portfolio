...
const ABOUT_TRANSLATIONS = {
  ...
  bn: {
    ...
    introPara1: "সিকো পাওয়ার লিমিটেড (এসপিএল) বাংলাদেশের জ্বালানি খাতের একটি শীর্ষস্থানীয় ইঞ্জিনিয়ারিং প্রতিষ্ঠান। উচ্চমানের ডিস্ট্রিবিউশন ও পাওয়ার ট্রান্সফরমার, বিবিটি (বাস বার ট্রাংকিং সিস্টেম), সুইচগিয়ার এবং ডিজেল জেনারেটরের ডিজাইন, উৎপাদন, টেস্টিং এবং সেবা প্রদানের মাধ্যমে আমরা নিজেদের গুণমান ও প্রকৌশল উৎকর্ষের একটি প্রতীক হিসেবে প্রতিষ্ঠিত করেছি।",
...
export default function AboutClient({ initialCerts }: AboutClientProps) {
  ...
  const [certs] = useState<any[]>(initialCerts);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [selectedTitle, setSelectedTitle] = useState<string>("");

  return (
    <div className="bg-[#FAF9F5] font-arone text-black min-h-screen pb-20 relative">
      ...
      <section className="py-16 px-6">
        <div className="mx-auto max-w-310">
          ...
          {/* Grid Layout representing credentials */}
            ...
            {certs.map((cert, index) => {
              ...
              return (
                ...
                >
                  ...
                  {cert.image ? (
                    <div
                      className="relative w-full aspect-4/3 rounded-xl overflow-hidden bg-neutral-100 shadow-xs border border-neutral-150 group cursor-zoom-in"
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <span className="p-3 bg-white/20 backdrop-blur-md rounded-full border border-white/20 text-white scale-90 group-hover:scale-100 transition-all duration-300">
      ...
      {/* Premium Lightbox Modal for Certificate Preview */}
      {selectedImage && (
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/85 backdrop-blur-md cursor-zoom-out p-4 transition-all duration-300"
          onClick={() => setSelectedImage(null)}
        >
          {/* Close button */}
          <button
            className="absolute top-6 right-6 p-3 rounded-full bg-white/10 text-white hover:bg-white/20 hover:scale-105 active:scale-95 transition-all select-none border border-white/10 cursor-pointer"
            onClick={() => setSelectedImage(null)}
          >
            <svg fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="h-6 w-6">
          ...
          <div
            className="relative max-w-4xl max-h-[85vh] w-full aspect-4/3 overflow-hidden rounded-2xl border border-white/10 bg-neutral-900 cursor-default shadow-2xl transition-all duration-300 scale-100"
          ...
          >
            <Image
              ...
              priority
            />
            {/* Overlay description bar */}
            {selectedTitle && (
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-6 text-white text-center">
                <h3 className="text-lg font-bold font-kanit">{selectedTitle}</h3>
              </div>
            )}
          </div>
        </div>
